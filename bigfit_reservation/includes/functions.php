...
function sec_session_start() {
	...
	session_regenerate_id (); // Erneuert die Session, lšscht die alte.
}
function login($email, $password, $mysqli) {
	...
	if ($stmt = $mysqli->prepare ( "SELECT id, username, password, salt
       ...
       WHERE email = ?
		...
		$stmt->execute (); // FŸhre die vorbereitete Anfrage aus.
		$stmt->store_result ();
		...
		if ($stmt->num_rows == 1) {
			// Wenn es den Benutzer gibt, dann wird ŸberprŸft ob das Konto
			...
			if (checkbrute ( $user_id, $mysqli ) == true) {
				...
				// †berprŸfe, ob das Passwort in der Datenbank mit dem vom
				// Benutzer angegebenen Ÿbereinstimmt.
...
function checkbrute($user_id, $mysqli) {
	...
	// Alle Login-Versuche der letzten zwei Stunden werden gezŠhlt.
	...
	if ($stmt = $mysqli->prepare ( "SELECT time
		...
		// FŸhre die vorbereitet Abfrage aus.
		$stmt->execute ();
		$stmt->store_result ();
